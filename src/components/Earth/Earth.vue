  ...
  <div :style="{ width: '100%', height: '100%' }" id="zf-earth">
    ...
    <div v-if="showMapTool">
      ...
      <div :class="{ toolLeftMore: toolLeftMore }">
        ...
        <base-map-panel v-if="BaseMapIf && showLeftTool" id="map-basemap"
          :class="mapToolHaveRight ? 'map-baseMap' : 'map-baseMapNoRight'" :data="baseMapPanelData">
        </base-map-panel>
      ...
      <div id="rightTool" :class="{ rightTool: true, toolRightMore: toolRightMore }">
        <!--   <toolbar
          ...
          class="toolbar1"
        ></toolbar> -->
        <layer-choice v-if="ToolIf && layerChoiceData != null" :mapData="layerChoiceData"
          :style="{ display: hideLayerChoice ? 'none' : 'block' }" @layerChange="layerChoiceChange">
        </layer-choice>
      </div>
      <!-- 图例 -->
      ...
      <div class="legendBox" v-if="legendShow">
        ...
        <div v-for="item in legendBoxData" :key="item.name">
          ...
          <div class="legendContent" v-if="item.type == 'Polygon'">
            <template>
              <div class="legendItem" v-for="(jtem, index) in item.legendDataArr" :style="{
                width: item.name == '预警等级' ? '100%' : '50%',
                paddingRight: item.name == '预警等级' ? '40px' : '0'
              }" :key="index">
                <div class="legendRenderBox"
                  :style="{ justifyContent: item.name == '预警等级' ? 'space-between' : 'start' }">
...
import { initLegacyMap, tryGetMapFacade } from "../../map";
...
export default {
  ...
  computed: {
    basemapShows() {
      return ((this.baseMapPanelData || { basemaps: [] }).basemaps).map(v => v.options.visible)
    }
  },
  watch: {
    basemapShows(newV, oldV) {
      this.$emit('changeBaseMap', this.basemapShows)
    }
  },
  data() {
    return {
  ...
  mounted() {
    // 初始化行政区代码：根据 localStorage 中的 xzqdm 设置
    // 000000 表示全国模式，不限制区域
    const localXzqdm = localStorage.getItem("xzqdm");
    if (localXzqdm && localXzqdm !== "000000") {
      _XZQDM = localXzqdm;
      mainConfig.regionCode = localXzqdm;
    }
    this.loadEarth();
  },
  methods: {
    ...
    loadEarth() {
      let me = this;
      let isLoaded = window.sessionStorage.getItem(me.earthLoadBsm);
      me.earth = new diitMap.Map("map", {
      ...
      if (window.localStorage.operationInfo != null) {
        ...
        if (mainConfig.regionCode != xzq && xzq != undefined) {
          me.earth._server.get(configurl).then(function (data) {
            me.earth.init(data);
          });
        } else {
          me.earth.init(configurl);
        }
      } else {
        me.earth.init(configurl);
      }

      //if (me.config != null) configurl = me.config;
      window.me = me;
      // 重构：挂载 MapFacade（Legacy 适配器），业务侧逐步改用 facade
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          console.warn("[MapFacade] legacy init skipped:", e && e.message);
        }
      }
      me.earth.eventManager.subscribe(
        me.earth.eventManager.enumEventType.Map.MapInitCompltedEvent,
        function (map) {
          //设置球已加载过
          window.sessionStorage.setItem(me.earthLoadBsm, "true");
          me._viewer = map.data.map;
          if (me.earth.MapType == 3) {
            me.drawControl = new DiitEarth.Draw(map.data.map, {
              hasEdit: false,
            });
          }
          //加载底图
          if (me.earth.dataConfig.defaultMap.basemaps != null) {
            for (
              let i = 0;
              i < me.earth.dataConfig.defaultMap.basemaps.length;
              i++
            ) {
              let op = me.earth.dataConfig.defaultMap.basemaps[i];
              if (op.options.visible == true) {
                op.options = op.options || {};
                op.options.Map = me.earth.map;
                let la = me.earth.layerManager.createLayer(
                  op.id,
                  op.type,
                  op.url,
                  op.options
                );
                me.earth.addBaseMapLayer(la);
              }
            }
          }

          //加载顶图
          if (me.earth.dataConfig.defaultMap.topmaps != null) {
            for (
              let i = 0;
              i < me.earth.dataConfig.defaultMap.topmaps.length;
              i++
            ) {
              let op = me.earth.dataConfig.defaultMap.topmaps[i];
          ...
          _MapControl["locateCity"] = function (xzqdm) {
            //发布行政区定位事件
            me.earth.eventManager.publish(
              me.earth.eventManager.enumEventType.Map.XZQHChangeEvent,
              {
                code: me.earth.enumMessageCode.Success,
                message: "",
          ...
          _MapControl["getParentXzqInfoByXzqdm"] = function (xzqdm) {
            if (_XZQDM.substr(-4) == "0000") {
              return me.$refs.regionLocateDom.getParentXzqInfoByXzqdm(xzqdm);
            } else {
              return me.getParentXzqInfoByXzqdm(xzqdm);
            }
          };
          ...
          me.earth.eventManager.subscribe(
            ...
            (XZQHChangeData) => {
              ...
              me.$emit("selectXzq", xzqdm, xzqmc);
            }
          );

          _MapControl.init(me.earth, me).then(() => {
            //mapControl初始化
            //工具条
            _ServerGet(me.earth.dataConfig.mapConfig.toolbarConfig)
              .then((res) => {
                me.toolbarData = {
                  domId: "map-toolbar",
                  myMap: me.earth,
            ...
            me.mapInitCompleted = true;

            //底图顶图切换
            if (!me.notShowBaseMap) {
              me.baseMapPanelData = {
                domId: "map-baseMap",
                myMap: me.earth,
                diitMap: diitMap,
                basemaps: me.earth.dataConfig.defaultMap.basemaps,
                topmaps: me.earth.dataConfig.defaultMap.topmaps,
                options: {
                  isShowBlankMap: true,
                },
              };
              me.BaseMapIf = me.showMapTool;
              me.BaseMapIfNoRight = me.mapToolHaveRight;
            }
            me.regionLocateData = {
              domId: "region-locate",
              myMap: me.earth,
            ...
            me.RegionLocateIf = me.showMapTool;
            ...
            // me.getLayer()
            me.$emit("onLoad", me.earth);
            me.$emit("earthLoaded", _MapControl);
            me.$nextTick(() => {
              me.delayFun();
            });
          });
    ...
    getParentXzqInfoByXzqdm(xzqdm) {
      let me = this;
      const root = me.$refs.regionLocateDom.LoadXZQHData.cities[0];
      const cities = me.$refs.regionLocateDom.LoadXZQHData.cities[0].childs;
      const xzqInfo = {};
