<template>
  <div class="legend-wrap">
        ...
        <div v-for="(Layers, $layerIndex) in Layer">
          <div v-for="(item, $index)  in Layers.LegendArry">
            <div v-for="(legend, lindex) in item" class="leg-item">
              <div class="lefticon">
                <div class="widget-legend-img" v-if="legend.img != null && legend.type !== 'Line'">
                  <img :src="legend.img" />
                </div>
                <div class="widget-legend-img"
                  v-if="legend.img == null && legend.color != null && legend.type !== 'Line'">
                  <div class="legend-color" v-if="legend.type == 'Point'"
                    :style="'border-radius:18px;height:24px;width:24px;background:'+legend.color"></div>
                  <div class="legend-color" v-if="legend.type == 'LineString'"
                    :style="'margin-top:8px;height:6px;width:48px;background:'+legend.color"></div>
                  <div class="legend-color" v-if="legend.type == 'Polygon' || legend.type == null"
                    :style="'border:'+legend.border+';background:'+legend.color"></div>
                </div>
              ...
              <div v-if="legend.type !== 'Line'" class="widget-legend-txt">{{ legend.label }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="diit-widget-drawer">
      <widgets-modal title="图例" :visible="visible && data.options.mode !== 'panel' " right="80px" top="100px"
        :modalProps="{
          width: '500px',
          mask: false,
          footer: null,
        ...
        <a-collapse @change="collapseChange">
          <a-collapse-panel v-for="(Layers, $layerIndex) in Layer" :key="String($layerIndex)"
            :header="Layer[$layerIndex].name">
            <a-Row v-for="(item, $index) in Layers.LegendArry" :key="$index">
              <a-Col span="8" v-for="(legend, lindex) in item" :key="lindex">
                <div class="widget-legend-img" v-if="legend.img != null">
                ...
                <div class="widget-legend-img" v-if="legend.img == null && legend.color != null">
                  <div v-if="legend.type == 'Point'"
                    :style="'border-radius:10px;height:20px;width:20px;background:'+legend.color"></div>
                  <div v-if="legend.type == 'LineString'"
                    :style="'margin-top:8px;height:5px;width:20px;background:'+legend.color"></div>
                  <div v-if="legend.type == 'Polygon' || legend.type == null"
                    :style="'height:20px;width:20px;background:'+legend.color"></div>
                </div>
                <div class="widget-legend-txt">{{ legend.label }}</div>
              </a-Col>
            </a-Row>
...
export default {
  ...
  methods: {
    ...
    getLegendJson(MapLayer) {
      ...
      if (that.myMap.dataConfig.apiConfig.apiIsProxy && url.indexOf("proxy") == -1) {
        ...
        success: function (data) {
          ...
          for (let i = 0; i < legendJson.layers.length; i++) {
            for (let j = 0; j < legendJson.layers[i].legend.length; j++) {
              var imgurl = MapLayer.url + "/" + legendJson.layers[i].layerId + "/images/" + legendJson.layers[i].legend[j].url;
              if (that.myMap.dataConfig.apiConfig.apiIsProxy && url.indexOf("proxy") == -1) {
